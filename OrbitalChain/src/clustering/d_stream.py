...
def demonstrate_dstream():
    print("=== D-Stream Clustering Test ===\n")
    ...
    # Generate synthetic data stream with 3 clusters
    np.random.seed(42)
    ...
    print("Generating streaming data with 3 clusters...\n")
    
    # Process points over time
    for t in range(100):
        # Generate points from random cluster
        cluster_idx = np.random.randint(0, 3)
        center = cluster_centers[cluster_idx]
        
        coords = center + np.random.randn(2) * 0.5
    ...
    print(f"\nCluster Details:")
    for cid, grids in clusters.items():
        center = centers.get(cid, np.array([0, 0]))
        print(f"  Cluster {cid}: {len(grids)} grids, center at ({center[0]:.2f}, {center[1]:.2f})")
    
    print("\nTrue cluster centers were:")
